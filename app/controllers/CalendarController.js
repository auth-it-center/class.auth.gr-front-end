(function () {
	...
	var CalendarController = function ($scope, $http, $compile, $routeParams, $location, $window, uiCalendarConfig) {
		...
		$scope.semesters = [];
		// List of semesters for current unit. Also depends on period being winter or spring
		$scope.eventSources = [];
		// Array of event sources. Will contain just the json feed object
		$scope.nEvents = 0;
		// Number of events for current unit and period
		$scope.resources = [];
		...
		$scope.unitInit = function (unitCode) {
			$scope.unitCode = unitCode;
			getUnitIdFromCode($scope.unitCode);
		};
		...
		$scope.jsonFeed = {
			...
			success: function (events) {
				...
				if ($scope.updateView) {
					//console.log("5 >>>> Jumping to date, scope.period is: " + $scope.period + ", and startDate: " + $scope.startDate + " value of startdate now: " + $scope.periodInfo[$scope.period - 1].startDate);
					calendar.fullCalendar('gotoDate', $scope.startDate); // new
					//calendar.fullCalendar('gotoDate', $scope.periodInfo[$scope.period - 1].startDate); // old
					$scope.updateView = false;
				}
			}
		...
		switch (requestedPath) {
			...
			case "/myCalendar":
				$scope.jsonFeed.url = "/personal/myCalendar";
				break;
			case "/calendar":
				$scope.jsonFeed.url = "/calendar/getCalendarEvents/";
				break;
		}
		...
		function getUnitList() {
			...
			var unitsUrl = "/calendar/getCalendarUnits/tree";
			// TODO: Hit real servers
			$http.get(unitsUrl).then(function (response) {
		...
		function getPeriodInfo() {
			...
			$http.get(periodsUrl).then(function (response) {
				...
				// console.log("2 >>>> Inside getPeriodInfo scope.period: " + $scope.period);
				$scope.startDate = $scope.periodInfo[$scope.period - 1].startDate;
			}, function () {
		...
		function updateSemesterList() {
			...
			if (requestedPath == "/calendar") {
				getUnitsDropDown();
				var start = 1;
				// Change this depending on current period
		...
		function checkUrlPersonal() {
			...
			if (Object.keys($routeParams).length === 2) {
				...
				if ($routeParams.period !== undefined && $routeParams.year !== undefined) {
					...
					getUnitPeriodInfo($scope.unitID);
				}
		...
		function checkUrlCalendar() {
			...
			if ($routeParams.unitId !== undefined && isNaN($routeParams.unitId)) {
				var codeList = $scope.units.map(function (unit) {
					return unit.code;
				});
				var idList = $scope.units.map(function (unit) {
					return unit.id;
				});
			...
			$scope.period = $scope.selectedPeriod.value[0];
			$scope.startDate = $scope.periodInfo[$scope.period - 1].startDate;
			// console.log("3 >>>> When checking url scope.period: " + $scope.period);
			$scope.year = $scope.selectedPeriod.value[1];
			...
			updateSemesterList();
			getUnitPeriodInfo($scope.unit.id);
		...
		function getTeacherCourses() {
			$scope.loading = true;
			$http.get("manage/getRegisteredCourses?period=" + $scope.period).then(function (response) {
				$scope.loading = false;
		...
		$scope.updateDuration = function () {
			$scope.loading = true;
			$http.get("manage/getEmptyRooms?date=" + $scope.date.format('DD-MM-YYYY HH:mm') + "&durationHours=" + $scope.durationHours + "&durationMinutes=" + $scope.durationMinutes).then(function (response) {
				$scope.loading = false;
		...
		function getUnitPeriodInfo(unit) {
			if (!unit) {
				$scope.loading = false;
				return
			}
			var unitPeriodsUrl = "/calendar/getUnitPeriods/" + unit;

			$http.get(unitPeriodsUrl).then(function (response) {
				if (response.data.length > 0) {

					// Iterate through unit period data to get startDate
					var unitPeriods = response.data;
					for (var i = 0; i < unitPeriods.length; i++) {
						if (unitPeriods[i].periodTypeId == $scope.period) {
							$scope.startDate = unitPeriods[i].startDate;
							break;
						}
					}
				}
			}, function () {
				console.error('Could not fetch unit period data!');
				$scope.loading = false;
			});
		...
		function getUnitIdFromCode(unitCode) {
			var unitCodeUrl = "/calendar/getUnitIdFromCode/" + unitCode;

			$http.get(unitCodeUrl).then(function (response) {
				if (response.data.length > 0) {
					$scope.unitID = response.data[0].unitCode;
				}
			}, function () {
				console.error('Could not fetch unit data!');
				$scope.loading = false;
		...
		$scope.periodChange = function periodChange() {
			...
			$scope.period = $scope.selectedPeriod.value[0];
			// console.log("4 >>>> On periodchange scope.period: " + $scope.period);
			$scope.year = $scope.selectedPeriod.value[1];
			updateSemesterList();
		...
		$http.get("/calendar/getPeriods").then(
			function (response) {
				...
				$scope.period = $scope.selectedPeriod.value[0];
				// console.log("1 >>>> Initially scope.period: " + $scope.period);
				$scope.year = $scope.selectedPeriod.value[1];
